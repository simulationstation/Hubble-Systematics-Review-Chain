...
PROG_JSON="${RUN_DIR}/perm_null_pkmjderr_within_survey_n5000.progress.json"
OUT_JSON="${RUN_DIR}/permutation_null__fields_pkmjd_err_pkmjd_err_within_survey_n5000.json"
...
exec taskset -c 0-127 env \
  ...
  "${ROOT_DIR}/.venv/bin/python" "${ROOT_DIR}/scripts/proxy_permutation_null.py" \
    ...
    --test-model "+fields+pkmjd_err" \
    --permute-column pkmjd_err \
    --apply-to cal \
    --mode within_survey \
    --n-perm 5000 \
    --seed 123 \
