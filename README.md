...
- **External `h0_grid` holdout (real data; new):** holding out each external constraint (training on SN-only+BAO+CC+ladder, testing on one `h0_grid:*` part) shows the **baseline** predicts these probes best; ladder-only correction models *reduce* predictive score (Δlogp < 0 in both diag and full-cov). This suggests the “winning” ladder corrections on calibrator holdouts do **not** generalize to external late-time H0 constraints.  
  Reports: `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_h0_holdout_realcal_diag_v1/report.md`, `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_h0_holdout_realcal_fullcov_v1/report.md`  
  Reproduce: `configs/stack_sn_bao_cc_plus_ladder_predictive_score_h0_holdout_realcal_diag_v1.yaml`, `configs/stack_sn_bao_cc_plus_ladder_predictive_score_h0_holdout_realcal_fullcov_v1.yaml`
...
- **CID holdout with calibrator duplicates *deduplicated* (real data; new):** enforcing one row per calibrator CID (choose max `FITPROB`) reduces the cross‑validated gains substantially. Diag scoring: Δlogp ≈ +0.21 (`cal_offset_bounded`) and ≈ +0.22 (`+bounded_fields_plus_metadata_bounded`) vs baseline. Full‑cov scoring: Δlogp ≈ +0.27 / +0.28 (same two models), i.e. the gain is **not** just a diagonal-scoring artifact. The top “hero” CID `2007af` drops from Δlogp≈+11.1 (n_test=4) to Δlogp≈+2.0 (n_test=1), implying earlier leverage was partly driven by duplicate survey reductions.  
  Reports: `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_v1/report.md`, `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_fullcov_v1/report.md`  
  Driver ranking: `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_v1/driver_ranking.md`, `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_fullcov_v1/driver_ranking.md`  
  Hero provenance: `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_v1/hero_calibrators_provenance.md`, `outputs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_fullcov_v1/hero_calibrators_provenance.md`  
  ...
  Reproduce: `configs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_v1.yaml`, `configs/stack_sn_bao_cc_plus_ladder_predictive_score_cid_holdout_realcal_surveytime_shoeslin_covproj_fragfilter_extgrid_more_cid_dedup_cal_bestfitprob_fullcov_v1.yaml`
